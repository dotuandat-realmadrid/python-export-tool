...
class AddProduct:
    ...
    def load_three_brands_tab(self):
        ...
        indicators = c.execute(f"SELECT id, requirement, indicator, unit, value FROM indicators WHERE type_id = ? AND id NOT IN ({placeholders})", 
        ...
        for index, ind in enumerate(indicators, 1):
            ...
            values = [index, wrap_text(req, 50), wrap_text(indc, 50)]  # SỬA: Tăng wrap_text lên 50 để hiển thị hết chữ dài
    ...
    def load_bom_tab(self):
        ...
        placeholders = ','.join('?' * len(hidden_ids)) if hidden_ids else '0'
        indicators = c.execute(f"SELECT id, requirement, indicator, unit FROM indicators WHERE type_id = ? AND id NOT IN ({placeholders})", 
                            (type_id,) + tuple(hidden_ids)).fetchall()
        ...
        for index, ind in enumerate(indicators, 1):
            ...
            values = [index, wrap_text(req, 50), wrap_text(indc, 50)]
    ...
    def load_ctkt_mua_sam_tab(self):
        ...
        # Xử lý "Yêu cầu khác" với STT là số nguyên
        ...
        sub_stt = 1  # Reset sub_stt cho custom items
    ...
    def load_dmkt_tab(self):
        ...
        req_number = 1  # SỬA: Biến đếm số nguyên cho header
        
        # SỬA: Nhóm indicators theo requirement để kiểm tra xem group có chỉ tiêu (indicator không rỗng) hay không
        groups = defaultdict(list)
        for ind in indicators:
            groups[ind[1]].append(ind)
        ...
        for req, ind_list in groups.items():
            ...
            # SỬA: Kiểm tra nếu group không có chỉ tiêu (tất cả indicator rỗng và chỉ có 1 item), thì hiển thị giá trị ngay tại header
            has_indicator = any(ind[2].strip() for ind in ind_list)  # Kiểm tra nếu có bất kỳ indicator nào không rỗng
            is_single_no_indicator = (len(ind_list) == 1 and not ind_list[0][2].strip())
            
            if is_single_no_indicator:
                # SỬA: Trường hợp không có chỉ tiêu: hiển thị header với giá trị, đơn vị, hành động (không thêm sub row)
                ...
                # SỬA: Display header là req (không thêm mô tả vì không có indicator)
                display_value = f"- {req}"
                
                values = [req_number, display_value, wrap_text(so_sanh or "", 20), wrap_text(unit or "", 20), "Xóa"]  # SỬA: STT là số nguyên
                ...
                height = line_count * 30
                max_height = max(max_height, height)
                req_number += 1  # Tăng số nguyên sau header
            
            else:
                # Trường hợp cũ: có chỉ tiêu (indicator không rỗng), thêm header rỗng, rồi sub rows
                values = [req_number, f"- {req}", "", "", "Xóa"]  # SỬA: STT là số nguyên
                line_count = 1
                item = self.dmkt_tree.insert("", "end", values=values, tags=("header",))
                self.dmkt_indicator_map[item] = ind_list[0][0]  # Gán id của first_ind cho header
                height = line_count * 30
                max_height = max(max_height, height)
                
                # SỬA: Thêm sub rows chỉ cho những ind có indicator không rỗng, với STT thập phân
                sub_stt = 1
                for ind in ind_list:
                    ind_id, req, indc, unit = ind
                    if not indc.strip():  # Bỏ qua nếu indicator rỗng (tránh duplicate nếu mixed)
                        continue
                    ...
                    if not so_sanh:
                        ...
                        if not bom_value:
                            hang_value = self.custom_indicators.get("three_brands", {}).get(f"so_sanh_{ind_id}", "")
                            if not hang_value:
                                danh_gia = self.custom_indicators.get("three_brands", {}).get(f"danh_gia_{ind_id}", "not")  # GIẢI THÍCH: Thêm lấy danh_gia để tính extreme cho sub_ind trong dmkt, giống add_product.py.
                                hang_value = self.calculate_extreme_value(ind_id, self.dmkt_tree, self.reference_columns, danh_gia)  # GIẢI THÍCH: Thay calculate_min_value bằng calculate_extreme_value để tính giá trị extreme dựa trên danh_gia, giống add_product.py.
                            bom_value = hang_value
                        so_sanh = bom_value
                    
                    # GIẢI THÍCH: Chỉ lấy chỉ tiêu, không thêm "không nhỏ hơn"
                    display_value = f"+ {indc}"
                    
                    values = [f"{req_number}.{sub_stt}", display_value, wrap_text(so_sanh or "", 20), wrap_text(unit or "", 20), "Xóa"]  # SỬA: STT thập phân
                    line_count = max([v.count('\n') + 1 for v in values if isinstance(v, str)])
                    ...
                    if not bom_reference:
                        hang_reference = self.custom_indicators.get("three_brands", {}).get(f"so_sanh_{ind_id}", "")
                        if not hang_reference:
                            danh_gia = self.custom_indicators.get("three_brands", {}).get(f"danh_gia_{ind_id}", "not")  # GIẢI THÍCH: Thêm lấy danh_gia để tính extreme cho sub_ind tham chiếu trong dmkt, giống add_product.py.
                            hang_reference = self.calculate_extreme_value(ind_id, self.dmkt_tree, self.reference_columns, danh_gia)  # GIẢI THÍCH: Thay calculate_min_value bằng calculate_extreme_value để tính giá trị extreme dựa trên danh_gia, giống add_product.py.
                        bom_reference = hang_reference
                    self.check_and_mark_row(self.dmkt_tree, item, ind_id, so_sanh, bom_reference, "dmkt")
                ...
                req_number += 1  # Tăng số nguyên sau mỗi group
    ...
    def load_ctkt_bo_tab(self):
        ...
        for req, ind_list in groups.items():
            ...
            has_indicator = any(ind[2].strip() for ind in ind_list)
            is_single_no_indicator = (len(ind_list) == 1 and not ind_list[0][2].strip())
            ...
            req_number += 1  # SỬA: Di chuyển lệnh tăng req_number ra ngoài nhánh điều kiện
