...
class DetailProduct:
    def __init__(self, root, product_id, parent=None):
        self.root = root
        self.product_id = product_id
        self.parent = parent
        self.root.title("Chi tiết sản phẩm")  # GIẢI THÍCH: Thay đổi tiêu đề từ "Chỉnh sửa sản phẩm" hoặc "Thêm sản phẩm mới" thành "Chi tiết sản phẩm" để phù hợp với yêu cầu hiển thị chi tiết sản phẩm đã chọn.
        ...
        tk.Button(root, text="Cập nhật sản phẩm", command=self.save_product).grid(row=0, column=2, padx=5, pady=5)  # GIẢI THÍCH: Thay đổi text của nút từ "Lưu sản phẩm" thành "Cập nhật sản phẩm" để phù hợp với chức năng cập nhật sản phẩm đã tồn tại, giữ nguyên command=self.save_product nhưng sẽ xử lý update trong hàm save_product.
        ...
        self.selected_type = None  # GIẢI THÍCH: Thay đổi từ self.selected_types = [] thành self.selected_type = None để chỉ cho phép chọn một loại sản phẩm duy nhất thay vì nhiều loại.
        ...
        self.column_widths = {
            "three_brands": {
                "STT": 50, "Yêu cầu kỹ thuật": 200, "Chỉ tiêu": 200, "Đánh giá": 100, "Giá trị": 150, 
                "Giá trị tham khảo": 150, "Đơn vị": 100, "Loại chỉ tiêu": 150, "Hành động": 100
            },  # GIẢI THÍCH: Thêm cột "Đánh giá": 100 vào column_widths của "three_brands" để phù hợp với giao diện add_product.py, nơi có cột "Đánh giá" trong tab Hãng.
            "bom": {
        ...
        self.load_types()
        self.load_product_data()  # GIẢI THÍCH: Loại bỏ kiểm tra if product_id vì trong DetailProduct luôn có product_id, nên gọi load_product_data() vô điều kiện để hiển thị dữ liệu sản phẩm đã chọn, bao gồm các sản phẩm tham khảo đã có.
    ...
    def init_default_crit_type(self):
        ...
        # GIẢI THÍCH: Lấy type_id từ self.selected_type
